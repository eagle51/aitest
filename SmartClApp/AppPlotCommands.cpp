...
void OdqCmd_Plot::execute(OdEdCommandContext* pCmdCtx)
{
  ...
  //OdGsModulePtr pGs = iVS->getGsModule(false, // temp workaround for missing GLES print support
  OdGsModulePtr pGs = iVS->getGsModule(true,
  ...
  if (pDbCtx->plotStyleType() == OdGiContext::kPsNone) // protected - if (!pDbCtx->m_pPlotStyleTable.get())
  {
    bool bPlotPlotstyle = pLayoutSettings->plotPlotStyles(),
         bShowPlotstyle = pLayoutSettings->showPlotStyles(); // = pLayoutInfo->showPlotStyles(pCurrLayout);
    ...
    if (pDbCtx->isPlotGeneration() ? bPlotPlotstyle : bShowPlotstyle)
    {
      ...
      if (!sFileName.isEmpty())
      {
        sFileName = pDwgDb->appServices()->findFile(sFileName);
        if (!sFileName.isEmpty())
        {
  ...
  if (pProperties->has(OD_T("QImageFrame")))
    pProperties->putAt(OD_T("QImageFrame"), OdRxVariantValue((OdIntPtr) &image));
  else
  {
