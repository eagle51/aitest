...
void OdqPlotDialog::ButtonBox_clicked(QAbstractButton* pButton)
{
  ...
  if (   sName == tr("Apply to Layout")
      || (sName == tr("OK") && bSave))
  {
    ODA_ASSERT_ONCE(m_pRxDatabase && !m_pPlotSettings.isNull());
    ...
    ODA_ASSERT_ONCE(   !m_pPlotSettings.isNull()
                    && pCurrLayoutSettings->objectId() != m_pPlotSettings->objectId());

    // apply m_pPlotSettings to current layout
    pCurrLayoutSettings->upgradeOpen();
    pCurrLayoutSettings->copyFrom(m_pPlotSettings);
...
void OdqPlotDialog::PlotStyles_currentIndexChanged(const QString& qsText)
{
  ...
  {
    resetPageSetupName();
    pValidator->setCurrentStyleSheet(m_pPlotSettings, sPlotStyle);
...
void OdqPlotDialog::fillDrawingOrientation()
{
  ...
  case OdDbPlotSettings::k0degrees: // No rotation
    qsImage += isPaperWidthLessHeight() ? "orientation_p.png" : "orientation_l.png";
  ...
  case OdDbPlotSettings::k90degrees: // 90?CW // CCW (for paper)
    qsImage += isPaperWidthLessHeight() ? "orientation_l.png" : "orientation_p.png";
    m_pLandscapeRadio->setChecked(true);
    m_pUpsideDownCheckBox->setCheckState(Qt::Unchecked);
    break;
  case OdDbPlotSettings::k180degrees: // Inverted
    qsImage += isPaperWidthLessHeight() ? "orientation_p_rev.png" : "orientation_l_rev.png";
    m_pPortraitRadio->setChecked(true);
    m_pUpsideDownCheckBox->setCheckState(Qt::Checked);
    break;
  case OdDbPlotSettings::k270degrees: // 90?CCW // CW (for paper)
    qsImage += isPaperWidthLessHeight() ? "orientation_l_rev.png" : "orientation_p_rev.png";
    m_pLandscapeRadio->setChecked(true);
    m_pUpsideDownCheckBox->setCheckState(Qt::Checked);
    break;
  }
