...
static QString aciColor(int index)
{
...
void OdqPropControlOdColor::setValue(const QVariant& value)
{
  ...
  ODA_ASSERT_ONCE(m_qsColor != QObject::tr("Custom...") && m_qsColor != "-1");
  //addColor(m_qsColor);
...
void OdqPropControlOdColor::addColor(const QString& qsColor)
{
  ...
  if (qsColor == QObject::tr("Custom..."))
  {
    icon = createIcon(m_qsColor);
    data = -1;
  }
  else
    icon = createIcon(qsColor);
...
QColor OdqPropControlOdColor::toQColor(const class OdCmEntityColor& color)
{
  if (color.colorMethod() == OdCmEntityColor::kByACI)
  {
    ...
    OdUInt8 blue = (rgb & 0xFF);
    OdUInt8 green = (rgb >> 8 ) & 0xFF;
    OdUInt8 red = (rgb >> 16) & 0xFF;
  ...
  if (color.isByLayer() || color.isByBlock())
  {
    ...
    IqView* iView = NULL;
    if (OdRxObject* pRxDb = getIConsole()->getCurrentDatabase())
      iView = getIViewSystem()->getView(pRxDb);
    if (iView)
    {
      ODCOLORREF clrBck = iView->backgroundColor(); getIAppProps()->toColorref(OD_T("Options/Display/ModelBackground"), ODRGB(0, 0, 0));
...
void OdqPropControlOdColor::onColorChanged()
{
  ...
  if (data == -1) // "Custom..."
  {
    ...
    OdCmEntityColor colorPrev = OdqPropertyModel::OdColorByString(m_qsColor);
    // restore prev index
    blockSignals(true);
    updateSelection();
    blockSignals(false);
    ...
    {
      QString qsColor = OdqPropertyModel::stringByOdColor(color);
      setValue(qsColor); // m_qsColor =
      setItemIcon(count() - 1, createIcon(getQColor()));
      updateSelection();
    }
    if (pThis.isNull())
      return;
...
QColor OdqPropControlOdColor::getQColor()
{
  OdCmEntityColor color = OdqPropertyModel::OdColorByString(m_qsColor);   
...
QIcon OdqPropControlOdColor::createIcon(const QString& qsColor)
{
  ...
  OdCmEntityColor color = OdqPropertyModel::OdColorByString(qsColor);
...
OdCmEntityColor OdqOdColorDialog::getColor(const OdCmEntityColor& colorInitial, 
                                           QWidget* pParent) // = NULL
{
  QColor qcolorInit = OdqPropControlOdColor::toQColor(colorInitial);
  ...
  pIndexEdit->setText(OdqPropertyModel::stringByOdColor(colorInitial));
  ...
  int indexTab = (colorInitial.colorMethod() == OdCmEntityColor::kByACI || pTabWidget->count() < 2) ? 0 : 1;
  ...
  if (pTabWidget->currentWidget() == pIndexTab)
  {
    ...
    bool bOk = false;
    OdCmEntityColor color = OdqPropertyModel::OdColorByString(qsColor, &bOk);
    if (!bOk)
      return OdCmEntityColor(OdCmEntityColor::kNone);
    return color;
  }
  ...
  if (pPicker)
  {
    //if (OdqPropertyModel::isVarious(index))
    {
          ...
          || OdqPropertyModel::getPossibleOdColors(values))
      {
        ...
        if (qsColor == QObject::tr("*VARIES*"))
          pPicker->addColor(qsColor);
        for (int iItem = 0; iItem < values.size(); iItem++)
        {
          QString qsColor_ = values[iItem];
          if (qsColor_ == QObject::tr("Custom..."))
            pPicker->addColor(qsColor);
          pPicker->addColor(qsColor_);
        }
        pPicker->addColor(qsColor);
                                ...
                                int role) const // = Qt::DisplayRole
{
  ...
  case Qt::DecorationRole: // render data as decoration (else as icon)
    ...
    if (type_ == IqAppProps::prpOdColor && index.column() == columnCount(index) - 1)
      return OdqPropControlOdColor::createIcon(pItem->value(index.column()));
    // TODO
                                    ...
                                    bool bRecreateWidgets) // = false
{
  ...
  OdqPropPaletteView* pTreeView = qobject_cast<OdqPropPaletteView*>(m_pWidget);
  if (pTreeView)
  {
...
#ifdef QT_DEBUG
  if (type(indexPage) == IqAppProps::prpPalette)
    indexPage = indexPage; // brk
#endif
                                          ...
                                          OdRxObject* pRxDb) // = NULL
{
  ...
  case OdCmEntityColor::kByACI: // index in palette "Index Color"
    qsColor = aciColor(color.colorIndex());
    break;
                                                  ...
                                                  bool* pIsOk) // = NULL
{
  ...
  *pIsOk = false;
  ...
  {
    if (qsColor != aciColor(indexColor))
      continue;
...
{
  if (!qsValue.compare("bylayer", Qt::CaseInsensitive))
