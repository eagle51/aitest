...
public:
  OdqPropDialogData(QWidget* pParent = NULL,
    ...
    : m_pDialog(new OdqPropertyDialog(pParent, flags))
  {
  }
  ...
  bool refreshSources(OdPropSource* pSourceOnly = NULL, // null for all
...
protected:
  ...
  QList<OdSharedPtr<OdqPropSourceData> > m_lstSrcData;
...
public:
  ...
  virtual bool getPossibleValuesXmlProps(OdDbStub* id, const OdStringArray& asClassNames,
                                         const OdString& sPathName, OdStringArray& values, OdStringArray& iconPaths);
  ...
  virtual bool transparencyByString(OdRxObject* pRxDb, const OdString& sTransparency, OdCmTransparency& transparencyDest);
...
protected:
  ...
  OdPropBasePtr createXmlPropByClassName(OdDbStub* id, const OdString& sClassName);
                                        ...
                                        void* pLayoutQObjectToAddDlg) // = NULL
{
  ...
  OdSharedPtr<OdqPropDialogData> pDlgData = new OdqPropDialogData(pDialogParent);
                                   ...
                                   const OdChar cDelimWidgetAttribs) // = __OD_T(':')
{
  ...
  if (pcszWidgetAttribs) 
  {
    ...
    foreach (QString qsAttrib, asAttribs)
    {
      ...
      if (!qsAttribLow.indexOf("type=palette"))
      {
                                        ...
                                        const OdString& sUnmergedBasePath) // = OdString::kEmpty // already set by previous call
{
  ...
  foreach (OdSharedPtr<OdqPropSourceData> pSrcData, pDlgData->m_lstSrcData)
  {
    ...
    bRes = pSrcData->setMergeFilter(sMergeFilter, sMergeFilterAttribName, sUnmergedBasePath);
  }
...
public:
  ...
  //  break;
    ...
    case OdDxfCode::HardOwnershipId:
      sValue.format(PRIX64W, (OdUInt64) m_pCurrent->getHandle());
...
OdPropBasePtr OdPropServicesImpl::createXmlPropByClassName(OdDbStub* id, const OdString& sClassName)
{
                                             ...
                                             OdDbStub* id, const OdStringArray& asClassNames)
{
  ...
  for (OdUInt32 index = 0, numClasses = asClassNames.size(); index < numClasses; index++)
  {
    ...
    OdPropBasePtr pPropSrc = createXmlPropByClassName(id, sClassName);
    if (pPropSrc.get())
    {
                                                   ...
                                                   OdStringArray& values, OdStringArray& iconPaths)
{
  ...
  for (uint index = 0, numClasses = asClassNames.size(); index < numClasses; index++)
  {
    ...
    if (pPropSrc.get())
    {
      if (pPropSrc->getPropertyPossibleValues(sPathName, values, iconPaths))
        return true;
    }
  }
  return false;
                                              ...
                                              OdPropSource::Action& action)
{
  ...
  for (uint index = 0, numClasses = asClassNames.size(); index < numClasses; index++)
  {
    ...
    if (pPropSrc.get())
    {
      ...
      {
        qsError = getIApp()->toQString(err.description()); // ? out of range
      }
      catch (const OdChar* psErr)
      {
        qsError = getIApp()->toQString(psErr); // ? some text about limits
      }
      catch (const char* pcszErr)
      {
        qsError = pcszErr; // ? some text about limits
      }
      if (!qsError.isEmpty())
...
OdSharedPtr<OdqXmlUtils>& OdPropServicesImpl::loadXmlPattern(const QString& cqsPatternFileName)
{
  ...
  if (!qsPatternFileName.endsWith(".cl", Qt::CaseInsensitive))
    qsPatternFileName += ".cl";
...
{
  bool isOk = false;
  OdCmEntityColor clr = OdqPropertyModel::OdColorByString(getIApp()->toQString(sColor), &isOk);
  if (!isOk)
    return false;
...
bool OdPropServicesImpl::transparencyByString(OdRxObject*, const OdString& sTransparency, OdCmTransparency& transparencyDest)
{
