...
ODRX_BEGIN_STATIC_MODULE_MAP()
  ...
  ODRX_DEFINE_STATIC_APPMODULE(L"SmartClViewSystem.tx", OdqViewSystemModule)
  ODRX_DEFINE_STATIC_APPMODULE(L"SmartClConsole.tx", OdqConsoleModule)
...
//#endif
  ...
  if (   !pModule
      ...
      && QFileInfo(qsModulePathName).fileName().indexOf("SmartClOpenGL") < 0)
    QMessageBox::question(
      QApplication::activeWindow(), 
      QObject::tr("SmartClApp Warning "),
...
//void OdqServices::checkProgress(int stepProgress, // = 0
...
void OdqServices::start(const OdString&) // displayString = OdString::kEmpty)
{
  //checkProgress();
}
...
//////////////////////////////////////////////////////////////////////////
//#if defined(Q_OS_WIN) || defined(Q_WS_WIN) // is obsolete for Q_OS_MAC and Q_OS_LINUX
                                 ...
                                 const OdString& sFilter) // = OdString::kEmpty
{
  ...
  if (!bForSave)
    qsSelectedFilter = settings.value("filter", filters.first()).toString();
...
#endif
  if (!bForSave && qsSelectedFilter != selFilter)
    settings.setValue("filter", selFilter);
