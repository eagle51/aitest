...
#include "../../SmartCl_Core/ClFactory.h"
...
static OdString getNewDrawingName()
{
	...
	QString qsRecentPath = settings.value(QString("%1/path").arg(qsSelectedExt),
	...
	{
		QString qsNewDrawingName = QString("JXY%1.dwg").arg(index);
...
void OdqCmd_New::execute(OdEdCommandContext* pCmdCtx)
{
	...
	//获取名称
	OdString sClName;
	OdSmartPtr<ExStringIO> pStartUpSiteIO = pCmdCtx->arbitraryData(OD_T("new/CLNAME"));
	...
	OdString sClType;
	OdSmartPtr<ExStringIO> pIO_ClType = pCmdCtx->arbitraryData(OD_T("new/CLTYPE"));
	if (pIO_ClType.get())
	{
	...
	QString strType = getIApp()->toQString(sClType);

	SmartCl::ClCate::Type clCateType = SmartCl::ClCate::strToType(
		strType.toLocal8Bit().toStdString());
	...
	// FILEDIA 设置为 1， 显示对话框。如果 FILEDIA 设置为 0，显示命令行提示
	...
	if (pDatabase.isNull())
	{
		pDatabase = pAppServices->createDatabase(true);
		//pDatabase->createLayout("Layout1");
...
//	OdString sNewDrawingName = getNewDrawingName();
	OdqConsoleTab& tab = *OdqConsole::getConsoleTab(pDatabase, true, sClName); // create and activate tab control
	...
	getIApp()->refreshQObjetcts("<event-refresh:drawing_history_changed>",
		getIApp()->toQString(sClName));

	IqViewSystem* iVS = NULL;

	bool aa = ::odrxDynamicLinker()->loadModule(OD_T("SmartClViewSystem")).isNull();
	bool bb = iVS = getIViewSystem();
	bool cc = !(iVS = getIViewSystem());

	if (::odrxDynamicLinker()->loadModule(OD_T("SmartClViewSystem")).isNull()
		|| !(iVS = getIViewSystem()))
	{
		sMsg = getIApp()->toOdString(
			QObject::tr("Module %1 was not found or invalid").arg("SmartClViewSystem.tx"));
	...
	if (pRxDatabase.isNull())
	{
		...
		if (!pExtDb)
		{
			...
			if (pPropServices.get()
				...
				// (Cryptlib is not distributed with Teigha. It must be obtained separately.
		...
		if (res == eDwgFileIsEncrypted || res == eDecryptionError)
			...
			// (Cryptlib is not distributed with Teigha. It must be obtained separately.
			//  See http://www.cryptlib.com for details.)
			qsMess += "\nCryptlib is not distributed with Teigha. Therefore encrypting is supported for Windows only.";
#endif
...
void OdqCmd_Recover::execute(OdEdCommandContext* pCmdCtx)
{
	...
	if (sPath.isEmpty())
	{
		...
		if (s_sFilters.isEmpty())
		{
			// fill filters for Teigha DWG only (DGN has not recovering functionality
			s_sFilters += L"DWG Drawing files (*.dwg)|*.dwg|";
			s_sFilters += L"DXF files (*.dxf)|*.dxf|";
	...
	if (sPath.isEmpty())
	{
		...
		if (pDwgDb.get())
		{
			...
			if (iRevSystem && !iRevSystem->isUnder(pRxDatabase))
			{
				...
				sFilters += sFlt = L"Teigha Format files (*.tig)|*.tig|";
			}
			sFilters += L"|";
		}
		else
		...
		if (pDwgDb.get())
		{
			...
			else if (sCurrFilter == L"Teigha Format files (*.tig)|*.tig|")
			{
...
void OdqCmd_Save::execute(OdEdCommandContext* pCmdCtx)
{
	...
	if (sFilePathName.isEmpty() || !isTitled)
	{
		OdqConsoleModule* pModule = static_cast<OdqConsoleModule*>(
			...
			::odrxDynamicLinker()->loadModule(OD_T("SmartClConsole")).get());
...
{
	ODA_ASSERT_ONCE(module());  // or via ::odrxDynamicLinker()->loadModule(OD_T("SmartClConsole.dll")).get()
...
void OdqCmd_Export::execute(OdEdCommandContext* pCmdCtx)
{
	...
	if (sPath.isEmpty())
	{
		...
		if (pos > 0)
			sPath = sPath.left(++pos) + s_sExtension;
		...
		//  way to use tracker for current filter transferring is bad too after 
		//  missing this argument in OdDbBaseHostAppServices::fileDialog declaration) 
		OdString sExtension = s_sCurrFilter; // see special support for it in CommandContextImpl::getFilePath and OdqServices::fileDialog
	...
	{
		ODA_ASSERT_ONCE(s_sExtension == sPath.mid(++pos));
	}
...
void OdqCmd_RenameView::execute(OdEdCommandContext* pCmdCtx)
{
	//OdString sPath;
	...
	//	QString strTitile = getIConsole()->getQTitle(pRxDb);
	//}
	////if (s_sExtension != OD_T("dwg") && s_sExtension != OD_T("dxf") && s_sExtension != OD_T("dxb"))
	////{
	////	ODA_FAIL_ONCE();
	...
	//////////////////////////////////////////////////////////////////////////
	OdString sName;
	sName = pCmdCtx->userIO()->getString(OD_T("重命名命令(旧名称=新名称)"));
	sName.trimLeft(' ');
	sName.trimRight(' ');
	int nFind = sName.find('=');
	;
	QString strOldName = getIApp()->toQString(sName.mid(0, nFind));
	OdString strNewName = sName.mid(nFind + 1, sName.getLength());

	QList<OdRxObject*> lstRxDatabases = getIConsole()->getOpenDatabases();
	foreach(OdRxObject* pRxDb, lstRxDatabases)
	{
		QString strTitile = getIConsole()->getQTitle(pRxDb);
		if (strTitile == strOldName)
		{
			OdqConsoleTab& tab = *OdqConsole::getConsoleTab(pRxDb); // create and activate tab control
			tab.setFilePathName(strNewName);
			IqViewSystem* iVS = getIViewSystem();
			iVS->updateViewTitles(pRxDb);
			//pTabWidget->setTabText(index, qsTabName);
			//getConsole().m_pTabWidget;
			//tab.settabt
		}
	}
}

void OdqCmd_RemoveView::execute(OdEdCommandContext* pCmdCtx)
{
	OdString sName;
	sName = pCmdCtx->userIO()->getString(OD_T("输入删除文档的名称："));
	QString strTitleName = getIApp()->toQString(sName);
	QList<OdRxObject*> lstRxDatabases = getIConsole()->getOpenDatabases();
	OdRxObject* pRxDatabase;
	foreach(OdRxObject* pRxDb, lstRxDatabases)
	{
		QString strTitile = getIConsole()->getQTitle(pRxDb);
		if (strTitile == strTitleName)
		{
			//getIConsole()->closeDatabase(pRxDb);
			pRxDatabase = pRxDb;
			break;
		}
	}
...
//编写命令模板
void OdqCmd_Xxxx::execute(OdEdCommandContext* pCmdCtx)
{
...
void OdqCmd_Test::execute(OdEdCommandContext* pCmdCtx)
{
	...
	if (indexKeyword == en_macLocale)
	{
		...
		pIO->putString(L"  ODA_LINKED_WITH_FRAMEWORK_FONDATION should be uncommented in SmartClConsole.pri\n");
#endif
		return;
	}
	//if (indexKeyword == ...)
