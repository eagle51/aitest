...
void OdqConsoleTab::clearSelectionSet()
{
  //executeCommand("ai_deselect", false);
    ...
    OdString moduleName(OD_T("SmartClConsole"));
#else
    OdString moduleName(OD_T("SmartClConsole.tx"));
#endif
      ...
      while (getTopCmdState().m_pStringIO.get() && !getTopCmdState().m_pStringIO->isEof())
      {
        ...
        try
        {
              ...
              && workingSelectionSet()) // && workingSelectionSet()->numEntities())
          {
            ...
            if (!pSysVarPE.isNull())
            {
              OdString sValue = pSysVarPE->valueToString(pCmdCtx, OD_T("PICKFIRST")).makeLower();
              ODA_ASSERT_ONCE(   sValue == OD_T("0") || sValue == OD_T("1") // DWG
                              || sValue == OD_T("false") || sValue == OD_T("true")); // DGN
              bool bValue = !(sValue.isEmpty() || sValue == OD_T("0") || sValue == OD_T("false"));
              if (bValue)
                getCmdContext().setPickfirst(workingSelectionSet());
            }
          }
