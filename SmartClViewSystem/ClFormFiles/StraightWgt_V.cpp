...
StraightWgt_V::StraightWgt_V(int clType, std::string clName) : StraightWgt(clType, clName)
{
	_cl_Stright_V = static_cast<Cl_Stright_V*>(_cl.GetPointee());
	connect(_insulatorParaBtn, SIGNAL(clicked()), this, SLOT(slotShowInsulatorParaDlg_V()));
}

StraightWgt_V::~StraightWgt_V()
{

}

void StraightWgt_V::readChildClDataToForm()
{
	readGapValueParaData();
}

void StraightWgt_V::writeChildClDataFromForm()
{
	writeGapValueParaData();
}

//@function: 
void StraightWgt_V::slotShowInsulatorParaDlg_V()
{
	writeDataFromForm();//保存界面数据到内核，在弹出绝缘子串对话框中可以计算角度

	InsulatorParaDlg_V dlg(_cl_Stright_V);
	dlg.exec();
}

//@function: 四种工况间隙值组合框   -- by mzq  2019/5/28 21:40
QGroupBox* StraightWgt_V::initGapValueGbx()
{
...
//@function:    -- by mzq  2019/5/28 21:41
void StraightWgt_V::readGapValueParaData()
{
	try
	{
		HdlMdl_GapValuePara_VL gapValuePara_VL = _cl_Stright_V->getDataUI_V()->getGapValuePara_VL();
...
//@function:    -- by mzq  2019/5/28 21:41
void StraightWgt_V::writeGapValueParaData()
{
	try
	{
		HdlMdl_GapValuePara_VL gapValuePara_VL = _cl_Stright_V->getDataUI_V()->getGapValuePara_VL();
...
void StraightWgt_V::writeReport()
{
	writeDataFromForm();
	Report_Straight_V report_V(_cl->getClName());
	report_V.wordReport();
}

//@function: 更新间隙推荐值   -- by mzq  2019/8/8 9:39
void StraightWgt_V::upDateSgstGap(const std::vector<std::string>& fourSugestGap)
{
	try
	{
