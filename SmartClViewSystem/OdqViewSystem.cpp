  ...
  , m_bCausticGsChecked(false)
{
  ...
  if (pMainWindow)
  {
    ...
    {
	  m_pMdiArea = new QMdiArea(pMainWindow);
      m_pMdiArea->setObjectName("MdiArea");
      pMainWindow->setCentralWidget(m_pMdiArea);
	//  m_pMdiArea->resize(QSize(m_pMdiArea->height(), m_pMdiArea->width()));//重新设置单文档窗口，避免文档挤压，显示不准确
    }
...
{
  return !getGsModule(true, false).isNull(); 
}

OdGsModulePtr OdqViewSystem::getGsModule(bool isGlesEnabled, // = true
                                         bool isCausticEnabled) // = true
{
  if (   isCausticEnabled 
      && ((m_bCausticGsChecked && m_pCausticGs.isNull())|| !getIAppProps()->toBool(OD_T("Vars/caustic"), false)))
...
//#endif
        ...
        //  { 
        //    if (OdString(L"QGLWidget") == pPropName)
        ...
        //  // TODO 
        //  //OdGsDevicePtr pDevice = m_pGlesGs->createDevice();
        //  //// pDevice->setProperty(L"QGLWidget", ...);
...
#        endif
  ...
  if (m_pGlGs.isNull())
  {
    getIConsole()->putString(QObject::tr("Module %1 was not found").arg("SmartClOpenGL.txv"));
#  ifdef Q_CC_MSVC
...
void OdqViewSystem::activateMdiSubWindow(QMdiSubWindow* pWindow)
{
  ...
  emit pView->activateTree(qsTitle);
                                            ...
                                            bool bDwgOnly) // = true
{
  ...
  OdqRenderArea* pRenderArea = qobject_cast<OdqRenderArea*>(pWidget);
  if (!pRenderArea)
    return NULL;

  OdqView* pView = pRenderArea->getView();
  ODA_ASSERT_ONCE(pView);
  ...
  point = pRenderArea->mapFromGlobal(point);
  return pView;
}
...
{
  bool isMaximized = true; // with the same way as in constructor of OdqView
  QMdiSubWindow* pSubWindow = getMdiArea()->currentSubWindow();
  if (pSubWindow)
    isMaximized = pSubWindow->isMaximized();
  else
  {
    //QSettings settings("ODA", QFileInfo(QCoreApplication::applicationFilePath()).baseName());
    //isMaximized = settings.value(TD_SHORT_STRING_VER_S "/view/maximized", true).toBool();
  }

  OdqView* pView = OdqView::create(m_pMdiArea, pRxDatabase, idLayoutView, pRect, isMaximized, enBusy_UninitializedGL);
  ...
  updateViewTitles(pRxDatabase, true);

#  if (QT_VERSION >= QT_VERSION_CHECK(5,0,0))
  if (isMaximized)
    QTimer::singleShot(0, pView, SLOT(showMaximized()));
  else
    QTimer::singleShot(0, pView, SLOT(show()));
#  else
  if (isMaximized)
...
//  }
//#endif
//  // useless // getMdiArea()->setOption(QMdiArea::DontMaximizeSubWindowOnActivation, false);
//
//  bool isRevOrder = false;
//  if (isMaximized && sz > 1)
//  {
//    isRevOrder = (QT_VERSION < QT_VERSION_CHECK(5, 0, 0));
//    int iRevOrder = getIAppProps()->toInt(L"Vars/dgnshowrev", -1);
//    if (iRevOrder >= 0 && iRevOrder < 2)
//      isRevOrder = (iRevOrder != 0);
//  }
//
//  QList<QPointer<OdqView> > lst, lstRev;
//  OdqView* pActiveView = NULL;
//  for (int index = 0; index < sz; index++)
//  {
//    OdDbStub* idLayoutView = idsLayoutViews[index];
//    OdqView* pView = OdqView::create(m_pMdiArea, pRxDatabase, idLayoutView, isMaximized, enBusy_UninitializedGL);
//
//    if (m_mapViews.end() == m_mapViews.insert(pRxDatabase, pView))
//    {
//      ODA_FAIL_ONCE();
//      continue;
//    }
//    lst << QPointer<OdqView>(pView);
//    lstRev.prepend(QPointer<OdqView>(pView));
//    if (!pActiveView) // INT-5534
//      pActiveView = pView; // first
//
//    if (index < (int) rects.size())
//    {
//      pView->adjustSize();
//      QRect rect = pView->geometry();
//      OdGsDCRect rectGS = rects[index];
//      OdGsDCPoint low = rectGS.m_min,
//                  hi = rectGS.m_max;
//      //->MoveWindow( low.x, hi.y, hi.x - low.x, low.y - hi.y );
//      rect = QRect(QPoint(low.x, hi.y), // topleft
//                   QPoint(hi.x, low.y)); // bottomright
//      pView->setGeometry(rect);
//    }
//  }
//
//  ODA_ASSERT_ONCE(pActiveView);
//  if (!pActiveView)
//    return NULL;
//
//  // test to show real :idx via lst
//  // (maybe better change order of view inside getVisibleLayoutViewIds)
//  //updateViewTitles(pRxDatabase, lst);
//  //
//  updateViewTitles(pRxDatabase); 
//
////#  if (QT_VERSION >= QT_VERSION_CHECK(5,0,0) && !defined(ANDROID)) // moved from OdqView::create
////  //QTimer::singleShot(0, pView, SLOT(showMaximized()));
////  if (isMaximized)
////    pView->setWindowState(Qt::WindowMaximized);
////  else
////    pView->setWindowState(Qt::WindowActive);
////  //pView->resize(600, 400);
////#  endif
//
//#if defined(ANDROID)
//  pActiveView->showMaximized();
//#else
//  foreach (QPointer<OdqView> pView,
//           isRevOrder ? lstRev : lst) // INT-5534 // lst) 
//  {
//    ODA_ASSERT_ONCE(!pView.isNull());
//#  if (QT_VERSION >= QT_VERSION_CHECK(5,0,0))
//    if (isMaximized)
...
//      QTimer::singleShot(0, pView, SLOT(show()));
//#  else
//    // TODO if rempve show from constructor OdqView and redesign next
//    if (isMaximized)
...
//  }
//#endif
//  m_pMdiArea->setActiveSubWindow(pActiveView);
//  return pActiveView;
//}
//#endif
...
bool OdqViewSystem::setActiveViewbyTitle(const OdString& sTitle)
{
  ...
  QString strTitle = getIApp()->toQString(sTitle);
  if (pActiveView && pActiveView->windowTitle() == strTitle)
    return true; // already done
  
  foreach (QPointer<OdqView> pView, m_mapViews.values())
  {
    ODA_ASSERT_ONCE(!pView.isNull());
	if (!pView.isNull() && pView->windowTitle() == strTitle)
    {
#if defined(ANDROID)
