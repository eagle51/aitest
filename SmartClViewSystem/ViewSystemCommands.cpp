...
void OdqCmd_Layout::execute(OdEdCommandContext* pCmdCtx)
{
  OdqViewSystemModule* pModule = static_cast<OdqViewSystemModule*>(
     ...
     ::odrxDynamicLinker()->loadModule(OD_T("SmartClViewSystem")).get());
...
static void view_layerState(OdEdCommandContext* pCmdCtx)
{
  ...
  case 0: // save
    ...
    OdDbLayerState::save(pDwgDb, sLSName, OdDbLayerState::kHidden|OdDbLayerState::kCurrentViewport);
    break;
  case 1: // delete
    if (!sLSName.isEmpty())
    {
...
void OdqCmd_GlesDlg::execute(OdEdCommandContext* pCmdCtx)
{
  ...
  bool bSvEnabled = getIAppProps()->toBool(L"Options/Display/ExGLES2CompositeMf", false);

  bool bEnabled = (0 == pIO->getKeyword(
                     OdString().format(L"Enable GLES2 Composite Metafiles Mode (Cur: %ls)[Yes/No]?",
                                       bSvEnabled ? L"Yes" : L"No").c_str(),
                     L"Yes No", 
                     bSvEnabled ? 0 : 1));
  if (bEnabled == bSvEnabled)
    return;

  getIAppProps()->set(L"Options/Display/ExGLES2CompositeMf", bEnabled);
  if (bEnabled)
  {
...
void OdqCmd_SaveWidgetData::execute(OdEdCommandContext* pCmdCtx)
{
	OdqViewSystem* pVS = (OdqViewSystem*)getIViewSystem();
	...
	QList<QMdiSubWindow *> SubWindowList = pVS->getMdiArea()->subWindowList();

	for (int iLoop = 0; iLoop < SubWindowList.size(); ++iLoop)
	{
		OdqView* pView = qobject_cast<OdqView*>(SubWindowList.at(iLoop));
		if (pView)
		{
			pView->saveWidgetData();
		}
	}
}

void OdqCmd_OpenWireParaDlg::execute(OdEdCommandContext* pCmdCtx)
{
	SmartCl::WPA_Dlg dialog(false);
	dialog.exec();
}

void OdqCmd_OpenMeteAreaDlg::execute(OdEdCommandContext* pCmdCtx)
{
	MeteAreaDlg tempMACDlg;
	tempMACDlg.hideText();
	tempMACDlg.exec();
}

void OdqCmd_OpenSgstGapDlg::execute(OdEdCommandContext* pCmdCtx)
{
	SgstGap_Dlg dialog;
	dialog.exec();
}

void OdqCmd_OpenInsulatorLib::execute(OdEdCommandContext* pCmdCtx)
{
	InsulatorLib_Dlg dialog;
	dialog.exec();
}

void OdqCmd_OpenWireAvgHLib::execute(OdEdCommandContext* pCmdCtx)
{
	WireAvgHLib_Dlg dialog(true);
	dialog.exec();
}

void OdqCmd_DrawCl::execute(OdEdCommandContext* pCmdCtx)
{
	OdqViewSystem* pVS = (OdqViewSystem*)getIViewSystem();
	OdqView* pView = qobject_cast<OdqView*>(pVS->getMdiArea()->currentSubWindow());
	pView->drawCl();
}

void OdqCmd_SaveCurrentCl::execute(OdEdCommandContext* pCmdCtx)
{
	OdqViewSystem* pVS = (OdqViewSystem*)getIViewSystem();
	OdqView* pView = qobject_cast<OdqView*>(pVS->getMdiArea()->currentSubWindow());
	pView->saveWidgetData();
